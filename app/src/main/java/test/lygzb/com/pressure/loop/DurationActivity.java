...
public class DurationActivity extends AppCompatActivity {
	...
	private void init(){
		if(!isAdd){
			editOnHour.setText(String.valueOf(duration.getOnTime().getHour()));
			editOnMinute.setText(String.valueOf(duration.getOnTime().getMinute()));
			editOnSecond.setText(String.valueOf(duration.getOnTime().getSecond()));
			editOffHour.setText(String.valueOf(duration.getOffTime().getHour()));
			editOffMinute.setText(String.valueOf(duration.getOffTime().getMinute()));
			editOffSecond.setText(String.valueOf(duration.getOffTime().getSecond()));
		}
	}
		...
		public void onClick(View v) {
			switch (v.getId()){
				case R.id.btn_save:
					...
					try {
						duration.getOnTime().setHour(Integer.parseInt(onHour));
						duration.getOnTime().setMinute(Integer.parseInt(onMinute));
						duration.getOnTime().setSecond(Integer.parseInt(onSecond));
						duration.getOffTime().setHour(Integer.parseInt(offHour));
						duration.getOffTime().setMinute(Integer.parseInt(offMinute));
						duration.getOffTime().setSecond(Integer.parseInt(offSecond));
					}catch (Exception e){
						e.printStackTrace();
					...
					if(isAdd){
						LoopHandler.getIns().getSelectedLoop().addDuration(duration);
					}
